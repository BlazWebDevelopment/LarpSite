      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 py-3">
          <div className="flex items-center gap-2 text-sm text-gray-500">
            ...
            <span>/</span>
            <span className="text-gray-700">{language === 'en' ? 'Culture' : '文化'}</span>
          </div>
        </div>
      </div>
                ...
                <div className="space-y-4">
                  {cultureArticles.map((article, index) => (
                    <Link key={article.id} href={`/article/${article.id}`} className="block group">
                      <div className={`flex gap-4 pb-4 ${index < cultureArticles.length - 1 ? 'border-b border-gray-100' : ''}`}>
          ...
          <aside className="hidden lg:block w-72 shrink-0">
            <div className="bg-white rounded-lg shadow-sm border border-gray-200">
              <div className="china-red-bar text-white px-4 py-2 font-bold text-sm rounded-t-lg">
                {language === 'en' ? 'Trending in Culture' : '文化热门'}
              </div>
              <div className="p-4">
